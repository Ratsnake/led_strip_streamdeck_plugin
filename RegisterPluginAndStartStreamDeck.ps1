...
$pluginProjectFile = "$basePath\led_strip_plugin.csproj"
...
If (-not (Test-Path $bindir)) {
  Write-Error "The output directory `"$bindir`" was not found.`n You must first build the `"led_strip_plugin`" project before calling this script.";
  exit 1;
}
